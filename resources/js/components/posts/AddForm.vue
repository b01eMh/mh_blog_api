<template>
  <form @submit.prevent="handleSubmit" class="mt-6 ml-4">
    <div>
      ...
      <input type="text" v-model="title" name="title" class="border-2 w-1/2 p-4 rounded-lg focus:outline-none focus:ring-2" placeholder="Enter title">
      <p v-if="errMsgTitle" class="text-red-500 text-sm italic">{{ errMsgTitle[0] }}</p>
    </div>
    <div class="mt-4">
      <label for="body" class="sr-only">Body</label>
      <textarea v-model="body" class="w-full focus:outline-none focus:ring-2 rounded-lg border-2" name="body" cols="30" rows="10"></textarea>
...
export default {
  ...
  methods: {
    ...
    async handleSubmit(){
      try{
        const response = await axios.post('/api/posts', { title:this.title, body:this.body })
